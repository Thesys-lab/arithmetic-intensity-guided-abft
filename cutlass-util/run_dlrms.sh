...
curdir=$(pwd)
#gemmdir=$CUTLASS_UTIL_DIR/gemm/mm
gemmdir=$CUTLASS_UTIL_DIR/gemm/dlrm
header="model,batch_size,count,M,N,K,AI,time_og,time_dot,slowdown_abft_thread,slowdown_abft_global_with_dot,slowdown_abft_global_nodot"
echo $header >> $outfile
echo $header

for batch_size in 1 256 512 1024 2048 4096; do
  for fil in $(ls $gemmdir/*.txt); do
    fil=$(echo $fil | awk -F'/' '{print $NF}')
    model=$(echo $fil | awk -F'_' '{print $1}')
    fullfil=$gemmdir/$fil
    idx=0
    fildir=$outdir/$model-batch_size_$batch_size
    if [ ! -d $fildir ]; then
      mkdir $fildir
    fi
    while IFS=' ' read -r M N K count; do
      ...
      ./tools/profiler/cutlass_profiler $cutlass_args --abft_next_kernelH=1 --abft_next_kernelW=1 | grep "," > $abft_global_file
      cd $curdir
      ...
      val="$model,$batch_size,$count,$batch_M,$N,$K,$ai,$gemm_time,$dot_time,$overhead_abft_thread,$overhead_abft_global,$overhead_abft_global_nodot"
      echo $val >> $outfile
      echo $val
